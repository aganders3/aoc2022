...
impl LL {
    fn shift(&mut self, i: usize, n: isize) {
        // remove the node
        let (old_prev, old_next) = self.map.remove(&i).expect("not in list?").clone();
        let (old_prev_prev, _) = self.map.get(&old_prev).expect("no old prev?").clone();
        let (_, old_next_next) = self.map.get(&old_next).expect("no old next?").clone();
        self.map.insert(old_prev, (old_prev_prev, old_next));
        self.map.insert(old_next, (old_prev, old_next_next));

        // new location
        let new_prev = if n > 0 {
            (0..n - 1).fold(old_next, |acc, _v| self.map.get(&acc).expect("can't chase fwd").1)
        } else if n < 0 {
            (0..n.abs()).fold(old_prev, |acc, _v| self.map.get(&acc).expect("can't chase rev").0)
        } else {
            old_prev
        };

        // reinsert the node
        let (new_prev_prev, new_next) = self.map.get(&new_prev).expect("no new prev?").clone();
        let (_, new_next_next) = self.map.get(&new_next).expect("no new next?").clone();
...
fn part_1(nums: &Vec<isize>, map: HashMap<usize, (usize, usize)>) -> isize {
    ...
    dbg!(nums[(0..1000).fold(dbg!(index_of_zero), |acc, _v| list.map.get(&acc).expect("can't chase fwd").1)])
    + dbg!(nums[(0..2000).fold(index_of_zero, |acc, _v| list.map.get(&acc).expect("can't chase fwd").1)])
    + dbg!(nums[(0..3000).fold(index_of_zero, |acc, _v| list.map.get(&acc).expect("can't chase fwd").1)])
}
...
fn part_2(nums: &Vec<isize>, map: HashMap<usize, (usize, usize)>) -> isize {
    ...
    (0..list.len).for_each(|i| {
        let i = i + index_of_zero;
        dbg!(i, nums[i % list.len]);
    });
...
fn main() {
    ...
    let (nums, map) = parse(include_str!("input.txt"));
    // println!("Part 2 {}", part_2(&nums, map));
}
